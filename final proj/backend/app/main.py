...
from app.models import Ecosystem, SimulateRequest, MitigationRequest
...
from app.engine.lockin_engine import compute_lockin, compute_exit_cost
...
@app.get("/api/risk/packages/{package_id}/lockin")
def get_package_lockin(package_id: str):
    """Returns the dependency lock-in score and exit cost for a specific package."""
    state.ensure_loaded()
    if package_id not in state.graph:
        raise HTTPException(status_code=404, detail="Package not found")
    lockin = compute_lockin(state.graph, package_id)
    exit_cost = compute_exit_cost(state.graph, package_id)
    return {"lockin": lockin, "exit_cost": exit_cost}
...
def evaluate_mitigations(req: MitigationRequest):
    ...
    # Apply transition risk budget constraints (P5)
    disruption_order = {"None": 0, "Low": 1, "Medium": 2, "High": 3}
    for s in strategies:
        within_budget = True
        budget_violations = []
        if req.max_disruption:
            if disruption_order.get(s["disruption"], 3) > disruption_order.get(req.max_disruption, 3):
                within_budget = False
                budget_violations.append(f"Disruption exceeds budget limit '{req.max_disruption}'")
        if req.protect_critical and s["critical_apps_exposed"] > 0:
            within_budget = False
            budget_violations.append(f"Exposes {s['critical_apps_exposed']} critical app(s)")
        s["within_budget"] = within_budget
    ...
    return {
        ...
        "budget_applied": {"max_disruption": req.max_disruption, "protect_critical": req.protect_critical},
    }
...
def dashboard_summary():
    state.ensure_loaded()
    risk_results = score_all_packages(state.graph)
    apps = [n for n, d in state.graph.nodes(data=True) if d.get("type") == "application"]
