...
def simulate_compromise(g: nx.DiGraph, pkg_id: str) -> dict:
    ...
    affected_apps = [
        {"id": n, "name": g.nodes[n]["name"], "criticality": g.nodes[n].get("criticality")}
        for n in visited if g.nodes[n]["type"] == "application"
    ]
    affected_services = [n for n in visited if g.nodes[n]["type"] == "service"]
    affected_packages = [n for n in visited if g.nodes[n]["type"] == "package"]
    ...
    for app in affected_apps:
        try:
            ...
            paths.append({
                ...
                "path": [{"id": n, "name": g.nodes[n]["name"], "type": g.nodes[n]["type"]} for n in path],
            })
        except nx.NetworkXNoPath:
            continue

    return {
        "compromised_package": {"id": pkg_id, "name": g.nodes[pkg_id]["name"]},
        "timeline": timeline,
